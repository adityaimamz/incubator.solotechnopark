...
function EditArticle() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosPrivate.put(`news/${uuid}`, formData, {
        headers: {
  ...
  const getArticleById = async (uuid) => {
    try {
      const response = await axiosPrivate.get(`news/detail/${uuid}`);
  ...
  return (
    <DashboardLayout>
      <h1 className="font-medium text-xl">Edit Data News</h1>
      ...
      <div className="bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div className="px-6 py-6 lg:px-8">
          <h3 className="mb-4 text-xl font-medium text-gray-900 dark:text-white">
            Edit article into our platform
          </h3>
          <form className="space-y-6" action="#">
            ...
            >
              {loading ? "Loading ..." : "Edit Article Now"}
            </button>
          </form>
        </div>
      </div>
