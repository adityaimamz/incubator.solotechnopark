...
function MentorEditModal() {
  ...
  const getMentorDataById = async (uuid) => {
    try {
      const response = await axiosPrivate.get(`mentor/${uuid}`);
      const data = response.data.data[0];

      setMentorName(data.name);
      setMentorPosition(data.position);
      setMentorImage(data.image);
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 -translate-y-2">
              ...
              <div className="px-6 py-6 lg:px-8">
                <h3 className="mb-4 text-xl font-medium text-gray-900 dark:text-white">
                  Edit your Mentor into our platform
                </h3>
                <form className="space-y-6" action="#">
                  ...
                  >
                    {loading ? "Loading ..." : "Edit Mentor Now"}
                  </button>
                </form>
              </div>
            </div>
