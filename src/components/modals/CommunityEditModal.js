...
function CommunityEditModal() {
  ...
  const handleFileChange = (event) => {
    ...
    setCommunityImageUrl(URL.createObjectURL(file)); // Membuat URL objek sementara
  };
  ...
  const getCommunityDataById = async (uuid) => {
    try {
      const response = await axiosPrivate.get(`community/${uuid}`);
      const data = response.data.data[0];

      setCommunityName(data.name);
      setCommunityPosition(data.path);
  ...
  useEffect(() => {
    if (state.isId) {
      getCommunityDataById(state.isId);
    }
  }, [state.isId]);
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 -translate-y-2">
              ...
              <div className="px-6 py-6 lg:px-8">
                ...
                <form className="space-y-6" action="#">
                  ...
                  >
                    {loading ? "Loading ..." : "Edit Community Now"}
                  </button>
                </form>
              </div>
            </div>
