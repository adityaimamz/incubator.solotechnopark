...
function TeamEditModal() {
  ...
  const [teamDesc, setTeamdDesc] = useState("");
  ...
  const handleFileChange = (e) => {
    ...
    setTeamImage(file);
    setTeamImageUrl(URL.createObjectURL(file)); // Membuat URL objek sementara
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosPrivate.put(`team/${state.isId}`, formData, {
        headers: {
  ...
  const getTeamById = async (uuid) => {
    try {
      const response = await axiosPrivate.get(`team/${uuid}`);
      const data = response.data.data[0];

      setTeamName(data.name);
      setTeamPosition(data.position);
      setTeamImage(data.image);
      setTeamdDesc(data.image);
                      ...
                      <div className="w-full h-[256px] bg-gray-50 overflow-hidden rounded-lg flex justify-center items-start">
                        {teamImage && (
                          <Image
                            ...
                            src={teamImageUrl ? teamImageUrl : teamImage}
                            alt="Image Viewer"
                            width={440}
                            height={250}
                          />
