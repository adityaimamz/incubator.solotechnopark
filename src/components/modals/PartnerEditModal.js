...
function PartnerEditModal() {
  ...
  const handleFileChange = (e) => {
    ...
    setPartnerImageUrl(URL.createObjectURL(file)); // Membuat URL objek sementara
  };
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("position", partnerPosition);
    formData.append("image", partnerImage);

    try {
      const token = Cookies.get("token");

      await axiosPrivate.put(`partner/${state.isId}`, formData, {
        headers: {
  ...
  const getPartnerId = async (uuid) => {
    try {
      const response = await axiosPrivate.get(`partner/${uuid}`);
      const data = response.data.data[0];

      setPartnerName(data.name);
      setPartnerPosition(data.position);
      setPartnerImage(data.logo);
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 -translate-y-2">
              ...
              <div className="px-6 py-6 lg:px-8">
                <h3 className="mb-4 text-xl font-medium text-gray-900 dark:text-white">
                  Edit Partner into our platform
                </h3>
                <form className="space-y-6" action="#">
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-center w-full gap-5">
                      <div className="w-full h-[256px] bg-gray-50 overflow-hidden rounded-lg flex justify-center items-start">
                        {partnerImage && (
                          <Image
                            ...
                            src={
                              partnerImageUrl ? partnerImageUrl : partnerImage
                            }
                            alt="Image Viewer"
                            width={440}
                            height={250}
                      ...
                      <label
                        ...
                        className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 text-center"
                      >
                        <div className="flex flex-col items-center justify-center pt-5 pb-6 p-2 overflow-hidden">
                          <svg
                            aria-hidden="true"
                  ...
                  >
                    {loading ? "Loading ..." : "Edit Partner Now"}
                  </button>
                </form>
              </div>
            </div>
